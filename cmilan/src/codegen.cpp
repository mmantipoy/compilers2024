...
// Функция обработки "весячих" меток
void CodeGen::handlingJumps(string labelName, int labelAddress, map<string, vector<int>> &hangingJumps) {

	// Проходимся по "весячим" меткам и ищем переданное название метки в списке
    for (int jumpAddress : hangingJumps[labelName]) {
		// Делаем операцию JUMP для метки по адресу метки
        emitAt(jumpAddress, JUMP, labelAddress);
    }

	// Удаляем метку из списка
    hangingJumps.erase(labelName);
}
