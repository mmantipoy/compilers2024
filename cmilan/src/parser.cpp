...
void Parser::parse()
{
	...
	// Проверяем что количество меток не больше количества GOTO поскольку обратная ситуация возможна
	// Это не является ошибкой компиляции, поэтому выводим предупреждение
	if ( gotoCount > 0 || labelTable.size() > 0){
		if ( static_cast<long unsigned int>(gotoCount) < labelTable.size() ) {
			cerr << "WARNING: label without corresponding GOTO" << endl;
		} 
	} 

	// Проверяем что после обработки всего кода не осталось "висячих" меток иначе это ошибка
	if ( hangingJumps.size() > 0 ) {
		reportError("missing labels.");
	}
...
void Parser::label() {
	...
	// Удаляем метку из потока
    mustBe(T_LABEL);
	// Получаем имя метки и ее адрес для операции JUMP
    string labelName = scanner_->getStringValue();
    int labelAddress = codegen_->getCurrentAddress();
	// Проверяем, есть ли уже метка с таким именем в таблице меток
	if ( labelTable.count(labelName) != 0 )
		reportError("multiple labels with the same name.");
	else{
		// Добавляем метку в таблицу меток и заполняем ее адресом
		labelTable[labelName] = labelAddress;
		// Проходимся по "весячим" меткам
		codegen_->handlingJumps(labelName, labelAddress, hangingJumps);
	}
}
...
void Parser::gotoStatement() {
	// Оператор из потока
    mustBe(T_GOTO);
	// Увеличиваем количество найденных GOTO
	gotoCount++;
	// Получаем имя метки на которую хотим совершить JUMP
    next();
    string labelName = scanner_->getStringValue();

	// Проверяем знаем ли мы уже такую метку или нет
    if (labelTable.count(labelName) == 0) {

		// Если нет, то добавляем ее в таблицу "весячих" меток и заполняем ее адресом
        int jumpAddress = codegen_->reserve();
        hangingJumps[labelName].push_back(jumpAddress);
    } else {

		// Если есть создаем команду JUMP для нее по сохраненному адресу
        int labelAddress = labelTable[labelName];
        codegen_->emit(JUMP, labelAddress);
    }
}
